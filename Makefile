...
KIND_CLUSTER ?= langfuse-controller-test-e2e
...
# architectures. Multi-arch images are built and pushed as a single manifest.
PLATFORMS ?= linux/arm64,linux/amd64
.PHONY: docker-buildx
docker-buildx: ## Build and push multi-arch docker image with verification
	@echo "Building multi-arch image for platforms: $(PLATFORMS)"
	@echo "Image: ${IMG}"
	$(CONTAINER_TOOL) buildx create --name langfuse-multiarch --use --driver docker-container || true
	$(CONTAINER_TOOL) buildx build --platform=$(PLATFORMS) --push --tag ${IMG} .
	@echo ""
	@echo "Verifying multi-arch manifest..."
	$(CONTAINER_TOOL) buildx imagetools inspect ${IMG}
	@echo ""
	@echo "✅ Successfully pushed multi-arch image!"
	$(CONTAINER_TOOL) buildx rm langfuse-multiarch || true
